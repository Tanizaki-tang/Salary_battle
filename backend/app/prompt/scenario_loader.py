...
DEFAULT_SCENARIO = _BACKEND_ROOT / "scenarios" / "scene_001_gm.md"


def _resolve_scenario_path(scenario_path: str | Path | None = None) -> Path:
    if scenario_path:
        return Path(scenario_path)
    env_path = os.getenv("SCENARIO_GM_PATH", "").strip()
    if env_path:
...
def load_opening_template(scenario_path: str | Path | None = None) -> str:
    ...
    return (
        "你好！感谢你参加了我们的面试，团队对你印象很好。"
        "我们想给你发一个后端开发的offer：月薪15K，14薪。"
        "公司现在A轮，发展很快，你觉得怎么样？"
    )
